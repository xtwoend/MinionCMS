...
<header class="header bg-light lter hidden-print bg-white box-shadow">
    <p> <strong>{{ __('admin::admin.pages') }}</strong></p>
    <div class="pull-right toolbar-control" style="margin-top: 10px;">
        <a href="{{ route('admin.pages.create') }}" class="btn btn-sm btn-success" data-bjax><i class="icon icon-plus"></i> {{ __('admin::admin.add-page') }}</a>
        <button data-action="refresh" class="btn btn-sm btn-info"><i class="icon icon-refresh"></i> {{ __('admin::admin.refresh') }}</button>
        <button data-action="delete" class="btn btn-sm btn-danger" data-url="{{ route('admin.pages.destroy', 0) }}"><i class="icon icon-trash"></i> {{ __('admin::admin.delete') }}</button>
    </div>
</header>
            ...
            <section class="scrollable hover wrapper w-f">
                <section class="panel panel-default box-shadow">
                    ...
                    <div class="table-responsive">
                        <table class="table table-striped m-b-none" data-ride="datatables" id="pages-table">
                            <thead>
                                <tr>
                                    <th>
...
$(function(){
    ...
    $('input[data-item="search"]').on('keypress keyup keydown', function(e) { 
        ...
        $('#pages-table_filter input').val(this.value);
        $('#pages-table_filter input').trigger(e.type, {'e': press});        
    });
});
</script>
@endsection