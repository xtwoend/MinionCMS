...
use Minion\Themes\Exceptions\FileMissingException;
...
class Themes
{
	 ...
	 */
	public function __construct(Filesystem $files, Repository $config, ViewFactory $viewFactory,  Finder $finder)
	{
		$this->config      = $config;
		$this->files       = $files;
    ...
    public function getThemeInfo($theme)
    {
		...
		if ($this->files->exists($path)) {
			...
			$attributes = Json::make($path)->toArray();
	...
	{	
		...
		return $this->viewFactory->make($this->getView($view), $data, $mergeData);
	}
	...
	protected function autoloadComponents($theme)
	{
		...
		$this->registerNamespace($activeTheme);
